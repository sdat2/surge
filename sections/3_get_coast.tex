...
\texttt{xarray}~\cite{hoyer2017xarray} is then be used to quickly extract
a series of NetCDF climate files into a single file for these points.
...
The primary coastline extracted was the eastern mainland coastline of the
United States (\texttt{eUS}),
...
border with North Korea (\texttt{vc}),
...
\texttt{eUS}. We define the symmetric derivative of this
smoothed angle,
...
bathymetry netCDF file that the model was run with (ETOPO2v2-derived in this
 ...
 points that are both level or below the isobath,
  and adjacent to a point above the isobath (Figure~\ref{fig:bath}).
